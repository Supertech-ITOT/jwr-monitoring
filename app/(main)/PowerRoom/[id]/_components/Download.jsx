"use client"
import PowerReport from '@/components/pdf/PowerReportPdf';
...
const Download = ({ id }) => {
    ...
    const handleDownload = async () => {
        ...
        const blob = await pdf(
            <PowerReport data={EnergyData} roomName={id} chartImg={dataUrl} category={roomCategory} />
        ).toBlob();

        const url = URL.createObjectURL(blob);
        const link = document.createElement("a");
        link.href = url;
        link.download = `PowerReport${id}`;
        link.click();
